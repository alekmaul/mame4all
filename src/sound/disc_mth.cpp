...
{
	PRECISION	iBias;		// current of the bias circuit
	PRECISION	exponent;	// smoothing curve
	PRECISION	rTotal;		// all resistors in parallel
};
...
struct dst_mixer_context
{
	...
	PRECISION	exponent_rc[DISC_MIXER_MAX_INPS];	// For high pass filtering cause by cIn
	PRECISION	vCap[DISC_MIXER_MAX_INPS];		// cap voltage of each input
	PRECISION	exponent_cF;				// Low pass on mixed inputs
	PRECISION	exponent_cAmp;				// Final high pass caused by out cap and amp input impedance
	PRECISION	vCapF;					// cap voltage of cF
	PRECISION	vCapAmp;				// cap voltage of cAmp
	PRECISION	gain;					// used for DISC_MIXER_IS_OP_AMP_WITH_RI
};
...
void dst_dac_r1_reset(struct node_description *node)
{
	...
	if (info->ladderLength < 2)
	{
		...
		printf("dst_dac_r1_reset - Ladder length too small");
	}
	if (info->ladderLength > DISC_LADDER_MAXRES )
	{
		printf("dst_dac_r1_reset - Ladder length exceeds DISC_LADDER_MAXRES");
	}
	...
	for(bit=0; bit < info->ladderLength; bit++)
	{
		if (!info->r[bit])
		{
			printf("dst_dac_r1_reset - Resistor can't equal 0");
		}
...
void dst_divide_step(struct node_description *node)
{
	if(node->input[0])
	{
		...
		{
#if PRECISION == float
			node->output=FLT_MAX;	/* Max out but don't break */
#else
			node->output=DBL_MAX;	/* Max out but don't break */
#endif
			printf("dst_divider_step() - Divide by Zero attempted.");
		}
		else
		{
...
void dst_mixer_step(struct node_description *node)
{
	...
	PRECISION	v, vTemp, rTotal, rTemp, rTemp2 = 0;
	PRECISION	i = 0;		// total current of inputs
	int	bit, connected;
...
void dst_mixer_reset(struct node_description *node)
{
	...
	{
		/* You must have an rF */
		printf("dst_mixer_reset - rF can't equal 0");
	}
	...
	if(node->input[0])
	{
		switch(context->clocktype)
		{
			...
			case DISC_SAMPHOLD_LLATCH:
				...
				break;
			default:
				//printf("dst_samphold_step - Invalid clocktype passed");
				break;
		}
	}
	else
...
PRECISION dst_transform_pop(PRECISION *stack,int *pointer)
{
	PRECISION value;
	//decrement THEN read
...
PRECISION dst_transform_push(PRECISION *stack,int *pointer,PRECISION value)
{
...
void dst_transform_step(struct node_description *node)
{
	if(node->input[0])
	{
		PRECISION trans_stack[MAX_TRANS_STACK];
		PRECISION result,number1,number2;
		...
		while(*fPTR!=0)
		{
			switch (*fPTR++)
			{
				...
				case '4':
					...
					break;
				default:
					//printf("dst_transform_step - Invalid function type/variable passed");
					node->output = 0;
					break;
			}
		}
