...
{
	PRECISION x1;		/* x[k-1], previous input value */
	PRECISION y1;		/* y[k-1], previous output value */
	PRECISION a1;		/* digital filter coefficients, denominator */
	PRECISION b0, b1;	/* digital filter coefficients, numerator */
};

struct dss_filter2_context
{
	PRECISION x1, x2;		/* x[k-1], x[k-2], previous 2 input values */
	PRECISION y1, y2;		/* y[k-1], y[k-2], previous 2 output values */
	PRECISION a1, a2;		/* digital filter coefficients, denominator */
	PRECISION b0, b1, b2;	/* digital filter coefficients, numerator */
};
...
struct dst_op_amp_filt_context
{
	...
	PRECISION	rTotal;		// All input resistance in parallel.
	PRECISION	iFixed;		// Current supplied by rP & rN if used.
	PRECISION	exponentC1;
	PRECISION	exponentC2;
	PRECISION	rRatio;		// divide ratio of rTotal & rF
	PRECISION	vC1;		// Charge on C1
	PRECISION	vC1b;		// Charge on C1, part of C1 charge if needed
	PRECISION	vC2;		// Charge on C2
	PRECISION	vF_last;	// Last output voltage relative to vRef.
	PRECISION	gain;		// Gain of the filter
};
...
static void calculate_filter1_coefficients(PRECISION fc, PRECISION type,
                                           PRECISION *a1, PRECISION *b0, PRECISION *b1)
{
	PRECISION den, w, two_over_T;
	...
	*a1 = (w - two_over_T)/den;
	...
	{
		printf("calculate_filter1_coefficients() - Invalid filter type for 1st order filter.");
	}
}
...
static void calculate_filter2_coefficients(PRECISION fc, PRECISION d, PRECISION type,
                                           PRECISION *a1, PRECISION *a2,
                                           PRECISION *b0, PRECISION *b1, PRECISION *b2)
{
	PRECISION w;	/* cutoff freq, in radians/sec */
	PRECISION w_squared;
	PRECISION den;	/* temp variable */
	PRECISION two_over_T = 2*Machine->sample_rate;
	PRECISION two_over_T_squared = two_over_T * two_over_T;
	...
	*a2 = (two_over_T_squared - d*w*two_over_T + w_squared)/den;
	...
	{
		printf("calculate_filter2_coefficients() - Invalid filter type for 2nd order filter.");
	}
}
...
{
	PRECISION f=1.0/(2*M_PI*node->input[2]*node->input[3]);
...
{
	PRECISION x1;		/* x[k-1], last input value */
	PRECISION y1;		/* y[k-1], last output value */
	PRECISION a1_0, b0_0, b1_0;	/* digital filter coefficients, filter #1 */
	PRECISION a1_1, b0_1, b1_1;	/* digital filter coefficients, filter #2 */
};

void dst_rcdisc2N_step(struct node_description *node)
{
	struct dss_rcdisc2_context *context = node->context;
	PRECISION input = ((node->input[0]==0)?node->input[1]:node->input[3]);
