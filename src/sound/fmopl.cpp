...
INLINE void CALC_FCSLOTOPL(OPL_CH *CH,OPL_SLOT *SLOT)
{
	int ksr;
...
INLINE void set_sl_rropl(FM_OPL *OPL,int slot,int v)
{
	OPL_CH   *CH   = &OPL->P_CH[slot/2];
	OPL_SLOT *SLOT = &CH->SLOT[slot&1];
	int sl = v>>4;
...
/* CSM Key Controll */
INLINE void CSMKeyControllOpl(OPL_CH *CH)
{
...
static void OPLWriteReg(FM_OPL *OPL, int r, int v)
{
	...
	case 0xa0:
		...
		if(CH->block_fnum != block_fnum)
		{
			...
			CALC_FCSLOTOPL(CH,&CH->SLOT[SLOT1]);
			CALC_FCSLOTOPL(CH,&CH->SLOT[SLOT2]);
		}
		return;
	case 0xc0:
		/* FB,C */
