...
{
	PRECISION sampleStep;	// time taken up by 1 audio sample
	PRECISION tCycle;		// time period of selected frequency
	PRECISION tLeft;		// time left sampling current frequency cycle
};

struct dss_lfsr_context
{
	unsigned int	lfsr_reg;
	PRECISION			sampleStep;
	PRECISION			shiftStep;
	PRECISION			t;
};
...
struct dss_op_amp_osc_context
{
	...
	int		type;
	int		is_squarewave;
	PRECISION	thresholdLow;	// falling threshold
	PRECISION	thresholdHigh;	// rising threshold
	PRECISION	iCharge[2];		// charge/discharge currents
	PRECISION	vCap;			// current capacitor voltage
	PRECISION	rTotal;			// all input resistors in parallel
	PRECISION	iFixed;			// fixed current athe the input
	PRECISION	step;
	PRECISION	temp1;			// Multi purpose
	PRECISION	temp2;			// Multi purpose
	PRECISION	temp3;			// Multi purpose
};
...
{
	PRECISION	ratioIn;		// ratio of total charging voltage that comes from the input
	PRECISION	ratioFeedback;	// ratio of total charging voltage that comes from the feedback
	PRECISION	vCap;			// current capacitor voltage
	PRECISION	rc;				// r*c
	PRECISION	exponent;
...
int	dss_lfsr_function(int myfunc,int in0,int in1,int bitmask)
{
	...
	switch(myfunc)
	{
		...
		case DISC_LFSR_REPLACE:
			...
			break;
		default:
			//printf("dss_lfsr_function - Invalid function type passed");
			retval=0;
			break;
	}
	return retval;
...
void dss_op_amp_osc_step(struct node_description *node)
{
	...
	PRECISION i;			// Charging current created by vIn
	PRECISION v = 0;		// all input voltages mixed
	PRECISION dt;			// change in time
	PRECISION vC;			// Current voltage on capacitor, before dt
	PRECISION vCnext = 0;	// Voltage on capacitor, after dt
...
void dss_op_amp_osc_reset(struct node_description *node)
{
	...
	PRECISION i1 = 0;	// inverting input current
	PRECISION i2 = 0;	// non-inverting input current
...
void dss_schmitt_osc_step(struct node_description *node)
{
	...
	PRECISION supply, vCap, new_vCap, t, exponent;
