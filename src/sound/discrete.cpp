...
int discrete_sh_start(const struct MachineSound *msound)
{
	...
	node_list = malloc(node_count * sizeof(node_list[0]));
	if (!node_list)
	...
	running_order = malloc(node_count * sizeof(running_order[0]));
	if (!running_order)
	...
	indexed_node = malloc(DISCRETE_MAX_NODES * sizeof(indexed_node[0]));
	if (!indexed_node)
	...
	dss_input_map = malloc(DSS_INPUT_SPACE * sizeof(dss_input_map[0]));
	if (!dss_input_map)
...
static void discrete_stream_update(int ch, INT16 **buffer, int length)
{
	...
	for (samplenum = 0; samplenum < length; samplenum++)
	{
		...
		{
			PRECISION val = output_node[outputnum]->input[0];
...
static void init_nodes(struct discrete_sound_block *block_list)
{
	...
	for (nodenum = 0; nodenum < node_count; nodenum++)
	{
		...
		{
			node->context = malloc(node->module.contextsize);
			if (!node->context)
