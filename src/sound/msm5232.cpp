...
typedef struct {
	...
	float	external_capacity[8]; /* in Farads, eg 0.39e-6 = 0.36 uF (microFarads) */
...
static void msm5232_init_tables( int which )
{
	...
	i = ((float)(1<<STEP_SH) * (float)chip->rate) / (float)chip->clock;
	chip->UpdateStep = i;
	/* logerror("clock=%i Hz rate=%i Hz, UpdateStep=%i\n",
			chip->clock, chip->rate, chip->UpdateStep); */

	scale = ((float)chip->clock) / (float)chip->rate;
	chip->noise_step = ((1<<STEP_SH)/128.0) * scale; /* step of the rng reg in 16.16 format */
	/* logerror("noise step=%8x\n", chip->noise_step); */

	for (i=0; i<8; i++)
	{
		float clockscale = (float)chip->clock / 2119040.0;
		chip->ar_tbl[i]   = ((1<<i) / clockscale) * (float)R51;
	}

	for (i=0; i<8; i++)
	{
		float clockscale = (float)chip->clock / 2119040.0;
		chip->dr_tbl[i]   = (     (1<<i) / clockscale) * (float)R52;
		chip->dr_tbl[i+8] = (6.25*(1<<i) / clockscale) * (float)R52;
	}
