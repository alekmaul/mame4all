...
struct dsd_555_astbl_context
{
	...
	PRECISION	vCap;		// voltage on cap
	PRECISION	step;		// time for sampling rate
	PRECISION	threshold;
	PRECISION	trigger;
};
...
struct dsd_566_context
{
	...
	PRECISION		vCap;			// voltage on cap
	PRECISION		step;			// time for sampling rate
	PRECISION		vDiff;			// voltage difference between vPlus and vNeg
	PRECISION		vSqrLow;		// voltage for a squarewave at low
	PRECISION		vSqrHigh;		// voltage for a squarewave at high
	PRECISION		thresholdLow;	// falling threshold
	PRECISION		thresholdHigh;	// rising threshold
	PRECISION		triOffset;		// used to shift a triangle to AC
};
...
void dsd_555_astbl_step(struct node_description *node)
{
	...
	PRECISION tRC;	// RC time constant
...
void dsd_555_cc_step(struct node_description *node)
{
	...
	PRECISION rC = 0;	// Equivalent charging resistor
	PRECISION rD = 0;	// Equivalent discharging resistor
	PRECISION vi = 0;	// Equivalent voltage from current source
	PRECISION vB = 0;	// Equivalent voltage from bias voltage
	PRECISION v  = 0;	// Equivalent voltage total from current source and bias circuit if used
	PRECISION dt;	// change in time
	PRECISION tRC;	// RC time constant
	PRECISION vC;	// Current voltage on capacitor, before dt
	PRECISION vCnext = 0;	// Voltage on capacitor, after dt
	PRECISION viLimit;	// vIn and the junction voltage limit the max charging voltage from i
	PRECISION rTemp;	// play thing
