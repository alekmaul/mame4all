...
void dss_adjustment_step(struct node_description *node)
{
	if (DSS_ADJUSTMENT_ENABLE)
	{
		...
		{
			PRECISION portval = (PRECISION)(rawportval - context->pmin) * context->pscale;
			PRECISION scaledval = portval * context->scale + context->min;
...
void dss_adjustment_reset(struct node_description *node)
{
	...
	context->pscale = 1.0 / (PRECISION)(node->input[6] - node->input[5]);
