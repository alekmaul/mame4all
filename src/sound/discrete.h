...
struct discrete_sound_block
{
	...
	PRECISION			initial[DISCRETE_MAX_INPUTS];	/* Initial values */
...
struct node_description
{
	...
	PRECISION			output;							/* The nodes last output value */

	int				active_inputs;					/* Number of active inputs on this node type */
	struct node_description *		
					input_node[DISCRETE_MAX_INPUTS];/* Either pointer to input node OR NULL in which case use the input value */
	PRECISION			input[DISCRETE_MAX_INPUTS];		/* Input values for this node */
...
struct discrete_schmitt_osc_desc
{
	...
	PRECISION	rFeedback;
	PRECISION	c;
	PRECISION	trshRise;	// voltage that triggers the gate input to go high (vGate) on rise
	PRECISION	trshFall;	// voltage that triggers the gate input to go low (0V) on fall
	PRECISION	vGate;		// the ouput high voltage of the gate that gets fedback through rFeedback
...
{
	int		type;
	PRECISION	cDefault;				// Default componet.  0 if not used.
	int		length;
	PRECISION	c[DISC_LADDER_MAXRES];	// Componet table
};
...
struct discrete_dac_r1_ladder
{
	...
	PRECISION	r[DISC_LADDER_MAXRES];	// Don't use 0 for valid resistors.  That is a short.
	PRECISION	vBias;			// Voltage Bias resistor is tied to (0 = not used)
	PRECISION	rBias;			// Additional resistor tied to vBias (0 = not used)
	PRECISION	rGnd;			// Resistor tied to ground (0 = not used)
	PRECISION	cFilter;		// Filtering cap (0 = not used)
};
...
struct discrete_mixer_desc
{
	int	type;
	int	mixerLength;
	PRECISION	r[DISC_MAX_MIXER_INPUTS];	// static input resistance values.  These are in series with rNode, if used.
	int	rNode[DISC_MAX_MIXER_INPUTS];	// variable resistance nodes, if needed.  0 if not used.
	PRECISION	c[DISC_MAX_MIXER_INPUTS];
	PRECISION	rI;
	PRECISION	rF;
	PRECISION	cF;
	PRECISION	cAmp;
	PRECISION	vRef;
	PRECISION	gain;				// Scale value to get output close to +/- 32767
};
...
struct discrete_555_astbl_desc
{
	...
	PRECISION	v555;			// B+ voltage of 555
	PRECISION	v555high;		// High output voltage of 555 (Usually v555 - 1.7)
	PRECISION	threshold555;	// normally 2/3 of v555
	PRECISION	trigger555;		// normally 1/3 of v555
};
...
struct discrete_555_cc_desc
{
	...
	PRECISION	threshold555;	// normally 2/3 of v555
	PRECISION	trigger555;		// normally 1/3 of v555
	PRECISION	vCCsource;		// B+ voltage of the Constant Current source
	PRECISION	vCCjunction;	// The voltage drop of the Constant Current source transitor (0 if Op Amp)
};
...
{
	int		options;	// bit mapped options
	PRECISION	vPlus;		// B+ voltage of 566
	PRECISION	vNeg;		// B- voltage of 566
};


struct discrete_adsr
{
	PRECISION attack_time;  /* All times are in seconds */
	PRECISION attack_value;
	PRECISION decay_time;
	PRECISION decay_value;
	PRECISION sustain_time;
	PRECISION sustain_value;
	PRECISION release_time;
	PRECISION release_value;
};
