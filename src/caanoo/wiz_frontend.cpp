...
static int show_options(char *game)
{
	...
	if (f) {
		...
		&wiz_frameskip,&wiz_sound,&wiz_clock_cpu,&wiz_clock_sound,&wiz_cpu_cores,&wiz_ramtweaks,&i,&wiz_cheat,&wiz_gsensor);
		fclose(f);
	}
	...
	while(1)
	{
        ...
        /* (11) Volume */
        if (wiz_gsensor)
            wiz_gamelist_text_out(x_Pos,y_Pos+120,"G-Sensor      ON");
        else
            wiz_gamelist_text_out(x_Pos,y_Pos+120,"G-Sensor      OFF");
		...
		wiz_video_flip();
		while(wiz_joystick_read(0)&0xFFFE00) { wiz_timer_delay(100000); }
		...
		else if(ExKey & WIZ_R || ExKey & WIZ_L)
		{
			...
			case 2:
				if(ExKey & WIZ_R)
				{
				    switch (wiz_video_aspect)
				    {
				        ...
				        case 9: wiz_video_aspect=0;break;
				    }
				}
				else
				{
				    switch (wiz_video_aspect)
				    {
				        case 0: wiz_video_aspect=9; break;
			...
			case 10:
				...
				break;
            case 11:
				wiz_gsensor=!wiz_gsensor;
                break;
			}
		}

		if ((ExKey & WIZ_A) || (ExKey & WIZ_B) || (ExKey & WIZ_MENU) || (ExKey & WIZ_SELECT)) 
		{
		...
		else if ((ExKey & WIZ_X) || (ExKey & WIZ_Y) || (ExKey & WIZ_HOME))
		{
			/* Return To Menu */
			return 0;
		}
...
static void wiz_exit(char *param)
{
	remove("frontend/mame.lst");
	sync();
	wiz_deinit();
	chdir("/usr/gp2x"); /*go to menu*/
	execl("gp2xmenu", "gp2xmenu", param, NULL);
}
...
static void select_game(char *emu, char *game)
{
	...
	while(1)
	{
		...
		if( (wiz_joystick_read(0)&0xFFFE00)) wiz_timer_delay(100000);
		while(!(ExKey=wiz_joystick_read(0)&0xFFFE00))
		{ 
			if (ExKey & WIZ_X)    wiz_exit("--view-game");
			if (ExKey & WIZ_HOME) wiz_exit("--view-main");
		}
