...
logerror("initialising MCU\n");
	...
	if (kikikai_mcu_initialised)
	{
		...
		{
			static const unsigned char db[16]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x08,0x00,0x10,0x18,0x00,0x00,0x00,0x00};
			unsigned short sy = mexico86_protection_ram[0xa0] + ((0x18)>>1);
			unsigned short sx = mexico86_protection_ram[0xa1] + ((0x18)>>1);

			for (i = 0; i < 0x38; i += 8)
			{
				unsigned char hw = db[mexico86_protection_ram[0x20 + i] & 0xf];

				if (hw)
				{
				    unsigned short xdiff = sx - ((UINT16)mexico86_protection_ram[0x20 + i + 6] << 8 | mexico86_protection_ram[0x20 + i + 7]);
				    if (xdiff < hw)
				    {
					unsigned short ydiff = sy - ((UINT16)mexico86_protection_ram[0x20 + i + 4] << 8 | mexico86_protection_ram[0x20 + i + 5]);
					if (ydiff < hw)
