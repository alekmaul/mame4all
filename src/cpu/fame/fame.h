...
#if defined (__cplusplus) //&& !defined(USE_FAME_C_CORE)
extern "C" {
#endif
...
unsigned famem68k_reset(void);
void     famem68k_emulate(int n);
unsigned famem68k_get_pc(void);
unsigned famem68k_get_cpu_state(void);
int      famem68k_fetch(unsigned address, unsigned memory_space);

/* Interrupt handling functions */
int  famem68k_raise_irq(int level, int vector);
int  famem68k_lower_irq(int level);
int  famem68k_get_irq_vector(int level);
int  famem68k_change_irq_vector(int level, int vector);

/* CPU context handling functions */
int  famem68k_get_context_size(void);
void famem68k_get_context(void *context);
void famem68k_set_context(void *context);
int  famem68k_get_register(m68k_register reg);
int  famem68k_set_register(m68k_register reg, unsigned value);

/* Timing functions */
unsigned famem68k_get_cycles_counter(void);
unsigned famem68k_trip_cycles_counter(void);
unsigned famem68k_control_cycles_counter(int n);
void     famem68k_release_timeslice(void);
void     famem68k_stop_emulating(void);
void     famem68k_add_cycles(int cycles);
void     famem68k_release_cycles(int cycles);
