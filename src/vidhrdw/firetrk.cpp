...
static struct osd_bitmap *helper1;
...
static void get_tile_info1(int tile_index)
{
	...
	//if (code != 0)
	    //debug_printf("ofst:%d tile:%x c:%d\n", tile_index, code & 0x3f, color);
}
...
static void get_tile_info2(int tile_index)
{
	...
	SET_TILE_INFO(2, code & 0x3f, color)
}
...
WRITE_HANDLER( firetrk_playfield_w )
{
	...
	{
		//debug_printf("ofst:%d code:%x c:%d\n", offset, data &0x3f, data >> 6);
		tilemap_mark_tile_dirty(tilemap1, offset);
...
int firetrk_vh_start(void)
{
	helper1 = bitmap_alloc(Machine->drv->screen_width, Machine->drv->screen_height);
	if (!helper1)
	    return 1;
	...
	tilemap_set_clip(tilemap1, &playfield_window);
	tilemap_set_clip(tilemap2, &playfield_window);
	...
	for (i = 0; i < 2; i++)
	{
		...
		for (y = 0; y < 256; y += Machine->gfx[0]->width)
		{
			drawgfx(bitmap, Machine->gfx[0], *p++, 0, 0, 0,
				x, y, &Machine->visible_area, TRANSPARENCY_NONE, 0);
		}
	}
}


void firetrk_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	int i;

	//tilemap_draw(bitmap, &playfield_window, tilemap1, 0, 0);
	tilemap_update(tilemap1);
	tilemap_render(tilemap1);
...
void firetrk_vh_eof(void)
{
	int i;

	//tilemap_draw(helper1, &playfield_window, tilemap2, 0, 0);
	tilemap_update(tilemap2);
	tilemap_render(tilemap2);
	tilemap_draw(helper1, tilemap2, 0);
